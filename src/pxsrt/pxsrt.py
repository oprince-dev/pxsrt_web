...
class PxSrt:
    ...
    def load_image_data(self):
        try:
            with Image.open(
                    "/home/oli/Projects/pxsrt_web/src/" + str(self.file)
                    ) as img:
    ...
    def save(self, flag):
        ...
        if flag == 'thresh':
            ...
            self.s_filename = '{}_{}{}{}{}{}{}'.format(base, self.mode,
                                                       self.direction,
                                                       self.threshold,
                                                       self.reverse,
                                                       self.upper,
    ...
    def generate_thresh(self):
        self.t_image = Image.fromarray(self.thresh_data, mode='HSV')
        self.t_image = self.t_image.convert('RGB')
        self.save("thresh")
    ...
    def partition(self, row, thresh_row):
        ...
        for p, t in zip(row, thresh_row):
            if t[2] == 255:
                partition_array = np.append(partition_array, np.array([p]),
                                            axis=0)
    ...
    def sort_pixels(self):
        ...
        self.s_image = Image.fromarray((sorted_pixels).astype(np.uint8))
        self.s_image = self.s_image.convert("RGB")
        self.save("sort")
